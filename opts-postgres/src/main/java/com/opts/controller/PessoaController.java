...
public class PessoaController {
    ...
    public Flux<PessoaEnt> getAll() {
        return pessoaService.findAll();
    }
/*
    @GetMapping(value = "/buscarId/{id}")
    public Mono<PessoaEnt> getById(@PathVariable String id) {
    ...
    public Mono<PessoaEnt> getByCpf(@PathVariable String cpf) {
        Mono<PessoaEnt> resultMono = pessoaService.findByCpf(cpf);
        return resultMono;
    }
    ...
    public ResponseEntity<List<PessoaDTO>> getByCep(@PathVariable String cep) {
        Flux<PessoaEnt> listFlux = pessoaService.findAll();
    ...
    public ResponseEntity<List<PessoaDTO>> getByNome(@PathVariable String nome) {
        Flux<PessoaEnt> listFlux = pessoaService.findAll();
        List<PessoaDTO> listDto = listFlux.toStream()
                .sorted(Comparator.comparing(PessoaEnt::getId).reversed())
    ...
    public Mono<PessoaEnt> save(@Validated @RequestBody PessoaEnt pessoaEnt) {
        return pessoaService.save(pessoaEnt);
    }

    @GetMapping(value = "/status", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<PessoaEnt> streamPessoaStatus() {
